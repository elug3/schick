...
- **Features:**
  ...
  - **NANO checkout bridge** (`GET /nano/checkout`) POSTs JSON + `API_KEY` server-side. Empty/unusable PG 2xx is `checkout_failed` (retryable); 3xx is forwarded as an absolute Location. The browser never POSTs to `request.io` (see [payment-service.md](payment-service.md))
