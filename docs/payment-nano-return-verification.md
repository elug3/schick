...
The value is percent-encoded in the JSON request so NANO's parser does not split on the raw `?`/`&` of those query params. NANO POSTs to the decoded URL. `compOrderMem` is the customer email truncated to 30 bytes (NANO's request field length).
